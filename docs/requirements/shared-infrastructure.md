...
- Child-session setup entries shall form a closed union keyed by `customType` for `taumel.childSession`, `taumel.permissions`, `taumel.plan`, and `taumel.plan_automation`, with each branch carrying its concrete payload contract. ^shared-9m6o
- Persisted Taumel session entries shall form a closed union over the canonical Pi custom-entry projection for child metadata, permissions, visibility, plans and plan automation, Ralph, agents, and cron, with each `customType` determining one concrete payload contract. ^shared-cyen
