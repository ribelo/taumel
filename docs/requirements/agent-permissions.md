...
- When a specialist or worktree-isolated child requests command escalation, the agent subsystem shall reject the request against the identity's immutable ceiling without presenting an approval prompt. ^agentperm-tqhg
- An exec-policy prompt for a specialist or worktree-isolated child may authorize an otherwise eligible command only within that identity's existing filesystem sandbox and shall keep the command sandboxed. ^agentperm-tg75
- Every approval decision and persisted exec-policy rule shall keep a specialist or worktree-isolated identity within its immutable read-only or isolation ceiling. ^agentperm-wo7i
