...
- The sandbox gateway shall provide one unforgeable internal `agent_worktree_mutation` authorization with the closed operation variants `provision`, `broker`, and `cleanup`. ^sandbox-ie72
- A validated `agent_worktree_mutation` effect shall contain its exact lifecycle operation, verified main repository, worktree path and administration area, dedicated branch ref and reflog, and shared object-store paths. ^sandbox-2llx
- The sandbox shall authorize `agent_worktree_mutation` only for the trusted worktree adapter performing the matching validated lifecycle operation within the identity's effective permission envelope; no model-facing tool, child shell, ordinary path mutation, or escalated command shall be able to construct, widen, or reuse that authorization. ^sandbox-hvys
- An `agent_worktree_mutation` authorization shall permit only the paths and Git operation encoded by that validated effect and shall not weaken protected-metadata handling for any other execution or mutation. ^sandbox-i1ev
