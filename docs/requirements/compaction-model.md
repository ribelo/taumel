...
- The system shall read `taumel.compaction.model` as `inherit` or a `provider/model` string from trusted project `<cwd>/.pi/settings.json` and global `~/.pi/agent/settings.json`, following the shared Taumel config precedence. ^compaction-cf01
- When the winning compaction-model value under the shared Taumel config precedence is `inherit`, the system shall defer to Pi's default compaction with the session model, and lower-precedence compaction-model scopes shall not apply. ^compaction-g442
...
- When the user runs `/compaction-model inherit`, the system shall set the session compaction model to inherit and, while the project is trusted, persist `inherit` to the project settings file, applying the same project persistence rule as `/compaction-model <provider/model>`. ^compaction-mwq7
