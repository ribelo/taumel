...
- Before native worktree creation, source-state reproduction, or baseline creation mutates worktree or shared Git metadata, the trusted worktree host adapter shall verify the provision operation's derived repository identity, managed path, dedicated branch, and collision state. ^agent-zywx
...
- On resume after process loss, the system shall inspect every retained provisional marker before permitting a conflicting creation; it shall retain unaccepted markers and block conflicting creation unless explicit rollback can verify and remove the managed path, native registration, and dedicated branch, and it shall never reclaim resources from marker fields alone. ^agent-4xub
...
- Before native Git mutates a worktree administration area, dedicated branch ref or reflog, or shared object store, the Agent Git broker shall verify the identity-owned worktree, native registration, expected main repository, and dedicated branch through the trusted host adapter. ^agent-nesg
...
- Before native worktree removal or provisional cleanup mutates worktree or shared Git metadata, the trusted worktree host adapter shall verify the derived identity-owned resources and shall report success only after the managed path, native registration, and intended branch satisfy the operation's cleanup postconditions. ^agent-85dy
