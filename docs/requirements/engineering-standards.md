...
- Taumel single-selection lists shall use Pi's `SelectList`, and Taumel mutable-settings lists shall use Pi's `SettingsList`. These lists shall use Pi's standard themes and built-in filtering, selection, navigation, scrolling, truncation, value presentation, and list hints. Feature-specific workflows shall be composed around the list component. Interactive managers that act on list rows in place rather than choosing a single entry shall use the shared modal kit instead of `SelectList`. ^eng-l2o2
...
- OCaml source formatting shall be defined by the repository's ocamlformat configuration; the configuration shall require exactly one blank line between consecutive top-level declarations, and contributions shall conform to the configured formatter. ^eng-0j8i
