...
- **viewmedia-h8s2** (event-driven): When `view_media` reads a valid supported image that exceeds the inline dimension or payload limits, the system shall resize the image before returning it to the model.
...
- **viewmedia-e3n9** (ubiquitous): The system shall perform `view_media` image decoding, resizing, and encoding in Taumel-owned execution code.
...
- Animated GIF/WebP frame-selection or frame-preservation semantics are not specified for v1.
