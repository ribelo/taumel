...
- **compaction-rs01** (ubiquitous): The system shall reuse Pi's exported `compact()` and `generateBranchSummary()` and swap only the model and its auth, inheriting Pi's summarization prompts, split-turn merge, file tracking, and structured summary formats.
- **compaction-hk01** (event-driven): When `session_before_compact` fires and a compaction model is configured, the system shall summarize through `compact()` with exactly that model and its auth and return the result to Pi.
- **compaction-hk02** (event-driven): When `session_before_tree` fires for a user-requested branch summary and a compaction model is configured, the system shall summarize through `generateBranchSummary()` with exactly that model and its auth and return the result to Pi.
...
- **compaction-cf04** (ubiquitous): The system shall treat missing, empty, and whitespace-only compaction-model values as absent, so an empty higher-precedence scope shall not shadow a configured lower-precedence scope.
...
- **compaction-er01** (unwanted): If the configured compaction model is invalid, absent from the registry, or lacks auth, then the system shall fail compaction visibly and shall not fall back to Pi's default compaction.
- **compaction-er02** (unwanted): If summarization through the configured compaction model fails, then the system shall fail compaction visibly and shall not retry with the session model.
- **compaction-er03** (unwanted): If branch-summary generation through the configured compaction model fails, then the system shall fail branch summarization visibly and shall not retry with the session model.
