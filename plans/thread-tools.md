...
- **threads-se04** (unwanted): The system shall not search, score, match, or return hidden reasoning payloads, encrypted reasoning content, raw full JSON event lines, or unbounded embedded tool-output blobs as search snippets, even if such fields contain the only occurrence of the query.
...
- **threads-rd07** (unwanted): The `read_thread` result shall not include hidden reasoning payloads, encrypted reasoning content, raw full JSON event lines, or unbounded embedded tool-output blobs, even when reading an exact event locator.
