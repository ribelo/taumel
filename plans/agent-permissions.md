...
- While Finder or Oracle has a read-only sandbox, when it requests command escalation under an owner approval policy that permits the request, the agent subsystem shall route the agent approval request through the same Pi-host approval flow used for a top-level tool call. ^agentperm-3fx1
