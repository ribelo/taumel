...
- **shared-st06** (ubiquitous): The system shall require explicit ownership for asynchronous exec sessions, child agents and runs, completion notifications, and cron delivery callbacks; ordinary synchronous component state shall use the loaded-session model instead of owner-keyed in-memory maps.
