...
- **render-df01** (ubiquitous): The system shall render `edit` and `apply_patch` as a real unified diff (via the `diff` package) with a dim right-aligned line-number gutter, colored `+`/`-`/space markers, a `(+N -M)` subject summary, changed lines plus 0–1 context capped near 6 collapsed, and full hunks expanded; `apply_patch` collapsed shows per-file `path (+x -y)` and expanded shows each file's full diff.
