...
let test_validation () =
  assert_bool "openai/gpt-4o is valid"
    (Compaction_model.is_valid_model_id "openai/gpt-4o");
  ...
  assert_bool "missing provider is invalid"
    (not (Compaction_model.is_valid_model_id "/gpt-4o"));
  assert_bool "missing model is invalid"
    (not (Compaction_model.is_valid_model_id "openai/"));
  assert_bool "no separator is invalid"
    (not (Compaction_model.is_valid_model_id "gpt-4o"));
...
let test_parse_command () =
  ...
  let set_ok =
    expect_ok "set" (Compaction_model.parse_command "openai/gpt-4o")
  in
  assert_bool "provider/model parses to Set"
    (set_ok = Compaction_model.Set "openai/gpt-4o");
  let nested_set_ok =
    expect_ok "set nested model"
      (Compaction_model.parse_command "openrouter/deepseek/deepseek-v4-pro")
  in
...
let test_resolve_empty_settings () =
  ...
  assert_bool "all empty settings use default"
    (Compaction_model.plan_session_before_compact empty_settings
...
let test_plan_command () =
  ...
  in
  let picker =
    expect_ok "picker inherits global"
      (Compaction_model.plan_command ~settings "")
  in
  assert_bool "empty command opens picker with current model"
    (picker
    = Compaction_model.Open_picker
        { current = Compaction_model.Model "openai/gpt-4o" });
  let set =
    expect_ok "set project"
      (Compaction_model.plan_command ~settings "anthropic/claude-3-5-sonnet")
  in
  ...
  let project_settings =
    {
      Compaction_model.session = None;
      global = Some "openai/gpt-4o";
      project = Some "anthropic/claude-3-5-sonnet";
    }
  in
  let show_project =
    expect_ok "picker prefers project"
      (Compaction_model.plan_command ~settings:project_settings "")
  in
  assert_bool "picker marks project model"
    (show_project
    = Compaction_model.Open_picker
        { current = Compaction_model.Model "anthropic/claude-3-5-sonnet" });
  let clear =
    expect_ok "clear project"
      (Compaction_model.plan_command ~settings:project_settings "clear")
  in
  assert_bool "clear plans project removal"
    (clear = Compaction_model.Clear_project);
  let clear_inherit =
    expect_ok "clear with no project shows inherit"
      (Compaction_model.plan_command ~settings "clear")
  in
  assert_bool "clear without project shows current"
    (clear_inherit
    = Compaction_model.Show_current
        { model = Compaction_model.Model "openai/gpt-4o"; source = "global" });
  let session_settings =
    { project_settings with session = Some "openai/gpt-5" }
  in
  let show_session =
    expect_ok "picker prefers session"
      (Compaction_model.plan_command ~settings:session_settings "")
  in
  assert_bool "picker marks session model"
    (show_session
    = Compaction_model.Open_picker
        { current = Compaction_model.Model "openai/gpt-5" });
  let empty_settings =
    { Compaction_model.session = None; global = None; project = None }
  in
  let show_empty =
    expect_ok "picker inherit"
      (Compaction_model.plan_command ~settings:empty_settings "")
  in
...
let test_plan_session_before_compact () =
  ...
  assert_bool "inherit means default"
    (Compaction_model.plan_session_before_compact inherit_settings
    = Compaction_model.Use_default);
  let global_settings =
    {
      Compaction_model.session = None;
  ...
  assert_bool "global model is used"
    (Compaction_model.plan_session_before_compact global_settings
    = Compaction_model.Use_model "openai/gpt-4o");
  let project_settings =
    {
      Compaction_model.session = None;
      global = Some "openai/gpt-4o";
  ...
  assert_bool "project model takes precedence"
    (Compaction_model.plan_session_before_compact project_settings
    = Compaction_model.Use_model "anthropic/claude-3-5-sonnet");
  let session_settings =
    { project_settings with session = Some "openai/gpt-5" }
  in
