...
let test_catalog () =
  ...
  List.iter
    (fun spec ->
      assert_bool
        ("network effect " ^ spec.Tool_gateway.name)
...
let test_missing_key_result () =
  ...
  assert_bool "missing key text" (String.contains (content_text result) 'E');
...
let test_crawling_preserves_full_text () =
  ...
  let text = content_text result in
  assert_bool "crawl text includes tail marker" (contains_substring text marker);
...
let test_search_preserves_requested_text () =
  ...
  assert_bool "search highlight visible"
    (contains_substring text "Important highlight");
...
let test_agent_run_structured_output_visible () =
  ...
  in
  let result =
    Exa.http_result ~tool_name:"exa_agent_get_run" ~status:200 ~body
  in
  let text = content_text result in
  assert_bool "structured output key visible" (contains_substring text "answer");
  assert_bool "structured output value visible"
    (contains_substring text "forty-two")
...
let test_agent_lists_expose_payload () =
  ...
  in
  let runs =
    Exa.http_result ~tool_name:"exa_agent_list_runs" ~status:200 ~body:runs_body
  in
  let runs_text = content_text runs in
  assert_bool "list runs exposes id" (contains_substring runs_text "run_1");
  assert_bool "list runs exposes status"
    (contains_substring runs_text "completed");
  assert_bool "list runs exposes cursor"
    (contains_substring runs_text "cursor_2");
  let events_body =
    {|{"data":[{"id":"evt_1","type":"agent_end","message":"done"}]}|}
  in
  let events =
    Exa.http_result ~tool_name:"exa_agent_list_events" ~status:200
      ~body:events_body
  in
  let events_text = content_text events in
  assert_bool "list events exposes id" (contains_substring events_text "evt_1");
  assert_bool "list events exposes type"
    (contains_substring events_text "agent_end")
