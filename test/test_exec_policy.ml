...
let assert_decision label expected actual =
  assert_equal label
    (Exec_policy.decision_to_string expected)
    (Exec_policy.decision_to_string actual)

let rule ?(decision = Exec_policy.Allow) ?(matches = []) ?(not_matches = [])
    pattern =
  Exec_policy.
    {
      raw_id = None;
...
let test_prefix_alternatives () =
  ...
  in
  let check =
    Exec_policy.decide_tokens compiled [ "git"; "reset"; "--hard"; "HEAD" ]
  in
  assert_decision "alternatives match" Exec_policy.Prompt check.decision;
  assert_equal "matched rule count" "1"
    (string_of_int (List.length check.matched_rules))
...
let test_no_match_defaults () =
    ...
    Exec_policy.compile
      [
        ( "global",
          [
            rule
              ~matches:[ Exec_policy.Tokens [ "git"; "reset" ] ]
              [ one "git"; one "reset"; one "--hard" ];
          ] );
      ]
  in
  assert_equal "invalid skipped" "0"
    (string_of_int (Exec_policy.active_rule_count invalid));
  ...
  assert_bool "error no-match overrides"
    (Exec_policy.override_decision (Exec_policy.decide_tokens invalid [ "pwd" ])
    = Some Exec_policy.Prompt)
    ...
    Exec_policy.compile
      [
        ( "global",
          [
            rule
              ~not_matches:[ Exec_policy.Tokens [ "rm"; "file" ] ]
              [ one "rm" ];
          ] );
      ]
  in
...
let test_ast_walk () =
  ...
  | Ok tokens ->
      assert_equal "walk tokens" "ls,-l,grep,ml" (String.concat "," tokens)
  ...
  let compiled =
    Exec_policy.compile
      [
        ( "global",
          [
            rule [ one "ls" ]; rule ~decision:Exec_policy.Forbidden [ one "rm" ];
          ] );
      ]
  in
  let ast =
...
let test_empty_policy_fallback () =
  ...
  assert_decision "dangerous fallback prompts under on-request"
    Exec_policy.Prompt
    (Exec_policy.decision_for_unmatched_command context
       [ "rm"; "-rf"; "target" ]);
  assert_decision "dangerous fallback prompts without sandbox"
    Exec_policy.Prompt
  ...
  assert_decision "dangerous fallback allowed under never" Exec_policy.Allow
    (Exec_policy.decision_for_unmatched_command
       ...
       [ "rm"; "-rf"; "target" ]);
  assert_decision "sudo dangerous fallback allowed under never full access"
    Exec_policy.Allow
    (Exec_policy.decision_for_unmatched_command
       {
         context with
  ...
  assert_bool "git status is safe"
    (Exec_policy.is_known_safe_command [ "git"; "status" ]);
...
let test_unsupported_defers_to_sandbox () =
  ...
  in
  assert_decision "unsupported construct runs under an active sandbox"
    Exec_policy.Allow
    (Exec_policy.decide_ast_with_fallback compiled sandbox_on unsupported)
      .decision;
  let yolo =
    Exec_policy.
      {
  ...
  in
  assert_decision "unsupported construct runs under never with no sandbox"
    Exec_policy.Allow
  ...
  assert_decision "unsupported construct prompts when escalation is requested"
    Exec_policy.Prompt
    (Exec_policy.decide_ast_with_fallback compiled escalated unsupported)
      .decision
...
let test_exec_policy_prompt_under_never_allows_but_boundaries_deny () =
  ...
  match
    Sandbox.authorize_mutation_path never_read_only Sandbox.Write "/repo/file"
  with
  | Sandbox.Deny _ -> ()
