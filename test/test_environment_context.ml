...
let test_full_context_serialization () =
  let snapshot =
    Env.snapshot ~cwd:"/repo" ~shell:"/bin/bash" full_access_sandbox
  in
  let xml = Env.serialize (Env.full snapshot) in
  assert_bool "context opens"
    (String.starts_with ~prefix:"<environment_context>" xml);
  assert_bool "context closes"
    (String.ends_with ~suffix:"</environment_context>" xml);
...
let test_diff_context_serialization () =
  ...
  | Some context ->
      ...
      assert_bool "diff includes writable root"
        (contains xml "<root>/repo</root>");
      assert_bool "diff omits shell" (not (contains xml "<shell>"))

let test_unchanged_context_has_no_diff () =
  let snapshot =
    Env.snapshot ~cwd:"/repo" ~shell:"/bin/bash" workspace_sandbox
  in
