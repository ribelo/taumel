...
let not_contains label haystack needle =
  ...
  if loop 0 then
    failwith
      (Printf.sprintf "%s: %S unexpectedly contains %S" label haystack needle)

let rendered label = function
  | File_read.Rendered r -> r
  | File_read.Binary_content ->
      fail label "expected Rendered, got Binary_content"
  | File_read.Out_of_bounds _ ->
      fail label "expected Rendered, got Out_of_bounds"

let () =
  (* Basic: line-number prefix, total/shown counts, no footer. *)
  let r =
    rendered "basic"
      (File_read.format ~content:"alpha\nbeta\ngamma\n" ~offset:None ~limit:None)
  in
  ...
  let r =
    rendered "linecap"
      (File_read.format ~content:(Buffer.contents buf) ~offset:None ~limit:None)
  in
  ...
  (* empty file. *)
  let r =
    rendered "empty" (File_read.format ~content:"" ~offset:None ~limit:None)
  in
