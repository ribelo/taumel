...
let test_gateway_enforces_profile_and_sandbox () =
  let spec = { Gateway.name = "exec_command"; effect_kind = Gateway.Execute } in
  ...
  in
  let denied_profile =
    Capability.resolve ~tools:Capability.None_allowed Capability.default
  in
  let allowed_context =
  ...
  match
    Gateway.authorize registry sandbox_denied_context ~name:"exec_command"
  with
...
let test_sandbox_patch_policy () =
  ...
  (match
     Sandbox.plan_write_stdin_host_call ~host_available:true ~yield_time_ms:250.
  ...
  | Sandbox.Stdin_result _ -> fail "stdin host plan" "expected host call");
  (match
     Sandbox.plan_write_stdin_host_call ~host_available:false
       { Sandbox.session_id = 7; chars = "q" }
   with
  | Sandbox.Stdin_result result ->
      assert_equal "stdin unavailable" Sandbox.write_stdin_unavailable_message
        result.message
...
let test_sandbox_workspace_metadata_protection () =
  ...
  assert_bool "host call shell args"
    (List.exists (fun arg -> arg = "-c") host_call.Sandbox.invocation.args);
  ...
  expect_approval_denied "approval denied by user"
    Sandbox.Approval_denied_by_user
    "Sandbox: command blocked (approval denied by user)" "denied_by_user";
  expect_approval_denied "approval timed out" Sandbox.Approval_timed_out
    "Sandbox: command blocked (approval timed out)" "timed_out";
  expect_approval_denied "approval unavailable" Sandbox.Approval_unavailable
    "Sandbox: command blocked (approval unavailable)" "unavailable";
  expect_approval_denied "approval interrupted" Sandbox.Approval_interrupted
    "Sandbox: command blocked (approval interrupted)" "interrupted";
...
let test_child_session_setup_entries () =
  ...
  | Shared.Object fields -> (
      match
        ( List.assoc_opt "activeTools" fields,
          List.assoc_opt "capabilityProfile" fields )
      with
      | Some (Shared.Array active_tools), Some profile_json ->
          ...
          let profile =
            expect_ok "ralph child profile" (Capability.of_json profile_json)
          in
  ...
  assert_equal "child parent id" "parent"
    (Option.value
       (Shared.json_string_field "parentSessionId" child.data)
       ~default:"");
  ...
  let steer_dispatch =
    Child_session.dispatch_plan ~bridge ~deliver_as:"steer"
      ~prompt:"steer child" ~send_available:true ()
  in
...
let test_plan_state_machine () =
  let plan =
    expect_ok "create plan"
      (Plan.add_user_task ~time_limit_seconds:5 ~session_id:"t1" ~now:1 " ship "
         None)
  in
  ...
  expect_error "update_plan complete rejected"
    (Plan.update_plan ~reason:"test transition" ~now:4 Plan.Complete (Some plan))

let test_ralph_ownership () =
  let task =
    expect_ok "start ralph"
      (Ralph.start ~max_iterations:1 ~id:"r1" ~controller_session:"controller"
         "iterate")
  in
  let task =
    expect_ok "attach child"
      (Ralph.attach_child (Ralph.Controller "controller") "child" task)
  in
  expect_error "controller cannot continue child"
    (Ralph.ralph_continue (Ralph.Controller "controller") task);
  let task =
    expect_ok "child continue" (Ralph.ralph_continue (Ralph.Child "child") task)
  in
  assert_int "iteration advanced" 1 task.iteration;
  assert_equal "child prompt"
    "Ralph task r1\n\
     Objective: iterate\n\n\
     Use ralph_continue with task_id r1 for each iteration, or ralph_finish \
     when complete."
    (Ralph.child_prompt task);
  let paused =
    expect_ok "max iterations pauses"
      (Ralph.ralph_continue (Ralph.Child "child") task)
  in
...
let test_thread_tools () =
                     ...
                     Shared.Object
                       [
                         ( "message",
                           Shared.Object
                             [
                               ("role", Shared.String "user");
                               ("content", Shared.String "current branch signal");
                             ] );
                       ];
                   ])
              ~entries:
                       ...
                       [
                         ...
                         ( "data",
                           Shared.Object
                             [
                               ( "summary",
                                 Shared.String "Current branch summary" );
                             ] );
                       ];
                   ]))
       with
  ...
  let thread_catalog : Threads.catalog =
    { threads = catalog; diagnostics = [] }
  in
...
let test_usage_openai_rendering () =
  ...
  let request =
    Usage.openai_usage_request ~account_id:"acct-test" ~token:"tok" ()
  in
  assert_equal "usage request url" "https://chatgpt.com/backend-api/wham/usage"
    request.url;
  ...
  assert_equal "usage request account header" "acct-test"
    (Option.value
       (List.assoc_opt "ChatGPT-Account-Id" request.headers)
       ~default:"");
  let credential =
  ...
  assert_bool "token lookup error"
    (Usage.token_state_of_lookup (Usage.token_lookup_from_host ~error:"boom" "")
  ...
  assert_equal "http error normalized"
    "OpenAI usage request failed: 429 Too Many Requests"
...
let test_tool_catalog_scope () =
  ...
  List.iter
    (fun name ->
      assert_bool ("omitted tool " ^ name) (not (Tool_catalog.has_tool name)))
    [
  ...
  let non_openai_sync =
    Tool_catalog.plan_active_tools_sync ~provider:"anthropic"
      [ "bash"; "bash"; "write" ]
  in
  assert_bool "non-openai active tool sync detects rewrite"
    non_openai_sync.changed;
  ...
  assert_bool "approval command omitted"
    (not (Tool_catalog.has_command "approval"));
  assert_bool "status command omitted" (not (Tool_catalog.has_command "status"));
  let notification =
    Tool_catalog.command_notification ~command_name:"usage" ~ok:false
      ~message:"" ~error:"network disabled"
  in
  ...
  (match
     Tool_catalog.plan_command_notification ~ui_available:true notification
   with
  ...
  (match
     Tool_catalog.plan_command_notification ~ui_available:false notification
   with
  | Tool_catalog.Notification_unavailable -> ()
  ...
  (match
     Gateway.text_result_json
       ~details:(Shared.Object [ ("ok", Shared.Bool true) ])
       "hello"
   with
  | Shared.Object fields ->
      assert_bool "tool result content"
        ...
        | Some (Shared.Array [ Shared.Object content_fields ]) ->
            ...
            && List.assoc_opt "text" content_fields
               = Some (Shared.String "hello")
        | _ -> false);
  ...
  assert_bool "full-access aliases danger-full-access"
    (Capability.sandbox_of_string "full-access"
    = Some Capability.Danger_full_access)
...
let test_permissions_state () =
  ...
  in
  let update =
    expect_ok "parse sandbox permission" (Permissions.parse "sandbox read-only")
  in
  let state =
    match update with
    | None -> fail "permissions parse" "expected update"
    | Some update ->
        expect_ok "apply sandbox permission"
          (Permissions.apply_update state update)
  in
  ...
  let state =
    match
      expect_ok "parse approval permission" (Permissions.parse "approval never")
    with
    | None -> fail "permissions approval parse" "expected update"
    | Some update ->
        expect_ok "apply approval permission"
          (Permissions.apply_update state update)
  in
  ...
  let state =
    match
      expect_ok "parse network permission" (Permissions.parse "network enabled")
    with
    | None -> fail "permissions network parse" "expected update"
    | Some update ->
        expect_ok "apply network permission"
          (Permissions.apply_update state update)
  in
  assert_bool "network enabled"
    (state.sandbox.network_mode = Sandbox.Network_enabled);
  let state =
    match
      expect_ok "parse full access permission"
        (Permissions.parse "sandbox full-access")
    with
    | None -> fail "permissions full access parse" "expected update"
    | Some update ->
        expect_ok "apply full access permission"
          (Permissions.apply_update state update)
  in
  ...
  expect_error "full access rejects network disabled"
    (Permissions.apply_update state
       (Permissions.Set_network Sandbox.Network_disabled));
  let state =
    match
      expect_ok "parse workspace permission"
        (Permissions.parse "sandbox workspace-write")
    with
    | None -> fail "permissions workspace parse" "expected update"
    | Some update ->
        expect_ok "apply workspace permission"
          (Permissions.apply_update state update)
  in
  ...
  let state =
    match
      expect_ok "parse no-sandbox permission"
        (Permissions.parse "no-sandbox enabled")
    with
    | None -> fail "permissions no-sandbox parse" "expected update"
    | Some update ->
        expect_ok "apply no-sandbox permission"
          (Permissions.apply_update state update)
  in
  ...
  (match
     expect_ok "parse network command" (Permissions.parse_network "enabled")
   with
  | Some (Permissions.Set_network Sandbox.Network_enabled) -> ()
  | _ -> fail "parse network command" "expected Set_network enabled");
  let state =
    expect_ok "deny tools"
      (Permissions.apply_update state Permissions.Deny_all_tools)
  in
  assert_bool "tool denied"
    (not (Capability.allow_tool state.profile "exec_command"));
  ...
  assert_bool "permissions menu excludes network"
    (Permissions.menu_selected_value permissions_menu
       "Network disabled (current)"
    = None);
  assert_equal "approval menu selection by label" "approval never"
    (Option.value
       (Permissions.menu_selected_value permissions_menu
          "Approval: never (current)")
       ~default:"");
  let network_menu = Permissions.network_menu_options state in
  assert_equal "network menu selection by label" "network disabled"
    (Option.value
       (Permissions.menu_selected_value network_menu
          "Network disabled (current)")
       ~default:"");
  (match
     Permissions.prompt_selection_plan ~ui_available:true ~title:""
       permissions_menu
   with
  | Permissions.Prompt_select plan ->
      assert_equal "permissions prompt title" Permissions.default_prompt_title
        plan.title;
