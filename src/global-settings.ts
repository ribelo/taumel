...
function nestedDiagnostics(root: SettingsObject, path: string): TaumelConfigDiagnostic[] {
  ...
  if (taumel !== undefined && !recordValue<SettingsObject>(taumel)) {
    return [diagnostic(path, "taumel", "taumel must be an object")];
  }
  if (!recordValue<SettingsObject>(taumel)) return diagnostics;
  for (const name of settingsBlocks) {
    const value = taumel[name];
    if (value !== undefined && !recordValue<SettingsObject>(value)) {
      diagnostics.push(diagnostic(path, `taumel.${name}`, `taumel.${name} must be an object`));
    }
  }
  const composer = recordValue<SettingsObject>(taumel["composer"]) ? taumel["composer"] : undefined;
  ...
  for (const name of visibilityBlocks) {
    const block = recordValue<SettingsObject>(taumel[name]) ? taumel[name] : undefined;
...
async function readRoot(path: string): Promise<{ exists: boolean; root: SettingsObject; diagnostics: TaumelConfigDiagnostic[] }> {
  try {
    ...
    return recordValue<SettingsObject>(parsed) ? { exists: true, root: parsed, diagnostics: [] } : { exists: true, root: {}, diagnostics: [diagnostic(path, "<root>", "global Pi settings must be a JSON object")] };
  } catch (error) {
    if (nodeErrorCode(error) === "ENOENT") return { exists: false, root: {}, diagnostics: [] };
...
export function parseTaumelGlobalSettings(value: unknown, path = taumelGlobalSettingsPath()) {
  const root = recordValue<SettingsObject>(value) ? value : undefined;
  ...
  const taumel = recordValue<SettingsObject>(root?.["taumel"]) ? root["taumel"] : {};
  const composer = recordValue<SettingsObject>(taumel["composer"]) ? taumel["composer"] : {};
...
export async function initializeTaumelGlobalConfig(path = taumelGlobalSettingsPath()): Promise<TaumelInitResult> {
  ...
  const taumel = recordValue<SettingsObject>(root["taumel"]) ? root["taumel"] : (root["taumel"] = {} as SettingsObject);
  const composer = recordValue<SettingsObject>(taumel["composer"]) ? taumel["composer"] : (taumel["composer"] = {} as SettingsObject);
  if (composer["enabled"] === undefined) {
    composer["enabled"] = true;
    initialized.push("taumel.composer.enabled");
  }
  for (const name of visibilityBlocks) {
    const block = recordValue<SettingsObject>(taumel[name]) ? taumel[name] : (taumel[name] = {} as SettingsObject);
