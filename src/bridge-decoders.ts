...
import { AgentWorktreeLineDeltaUpdateSchema, type AgentWorktreeLineDeltaUpdate } from "./bridge-action-contracts.ts";
...
const skillExpansionEffectsDecoder = Compile(SkillExpansionEffectsSchema);
...
const bridgeErrorResultDecoder = Compile(BridgeErrorResultSchema);
...
const agentWorktreeLineDeltaUpdateDecoder = Compile(AgentWorktreeLineDeltaUpdateSchema);
...
export function decodeSkillExpansionEffects(value: unknown): SkillExpansionEffects {
  try { return skillExpansionEffectsDecoder.Decode(value); }
  catch { throw new Error("Invalid OCaml skill-expansion effects"); }
}
...
export function decodeChildPermissionRefreshPlan(value: unknown): ChildPermissionRefreshPlan {
  try { return childPermissionRefreshPlanDecoder.Decode(value); }
  catch { throw new Error("Invalid OCaml child-permission refresh plan response"); }
}

export function decodeSessionCustomEntry(value: unknown): AnyPersistedTaumelCustomEntry {
  try {
    const entry = sessionCustomEntryDecoder.Decode(value);
    if (entry.customType === "taumel.cron") {
      const ids = new Set<string>();
      for (const task of entry.data.tasks) {
        if (ids.has(task.id)) throw new Error("duplicate cron task id");
        ids.add(task.id);
        if (task.nextDue <= task.createdAt) throw new Error("invalid cron timestamps");
  ...
  catch { throw new Error("Invalid Taumel custom session entry"); }
}
...
export function decodeCoreAck(value: unknown): CoreAck {
  try { return coreAckDecoder.Decode(value); }
    ...
    let failure: { readonly error: string };
    try { failure = bridgeErrorResultDecoder.Decode(value); }
    catch { throw new Error("Invalid OCaml acknowledgement"); }
    throw new Error(failure.error);
  }
}
export function decodeAuthorityPlanIssued(value: unknown): AuthorityPlanIssued {
  try { return authorityPlanIssuedDecoder.Decode(value); }
  catch { throw new Error("Invalid OCaml authority plan response"); }
}
...
export function decodeAgentWorktreeLineDeltaUpdate(value: unknown): AgentWorktreeLineDeltaUpdate {
  try { return agentWorktreeLineDeltaUpdateDecoder.Decode(value); }
  catch { throw new Error("Invalid agent worktree line-delta update"); }
}
