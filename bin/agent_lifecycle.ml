...
let record_child_session_start_result facts ctx =
  ...
  let agent_id =
    Tool_contracts.RecordAgentChildSessionStartFacts.get_agent_id facts
  in
  let child_session_id =
    Tool_contracts.RecordAgentChildSessionStartFacts.get_sessionId facts
  in
  let child_session_file =
    Tool_contracts.RecordAgentChildSessionStartFacts.get_sessionFile facts
  in
  match
...
let record_child_session_start_authorized facts capability ctx =
  ...
  in
  let agent_id =
    Tool_contracts.RecordAgentChildSessionStartFacts.get_agent_id decoded
  in
  match
    Agent_action_capability.revalidate_transition_result ~agent_id capability
      ctx
  with
  | Error message -> error_obj message
  | Ok () -> (
      ...
      | Error message -> error_obj message
      | Ok () ->
          Agent_action_capability.complete_transition_result ~agent_id
            capability ctx
          |> ack_result)

let rollback_unaccepted_start facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.RollbackUnacceptedAgentStartFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let agent_id =
    Tool_contracts.RollbackUnacceptedAgentStartFacts.get_agent_id facts
  in
  let run_id =
    Tool_contracts.RollbackUnacceptedAgentStartFacts.get_run_id facts
  in
  let submission_id =
    Tool_contracts.RollbackUnacceptedAgentStartFacts.get_submission_id facts
  in
  match
...
let rollback_send_preflight facts ctx =
  let facts =
    decode_ojs_contract
      Boundary_contracts.RollbackAgentSendPreflightFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let agent_id =
    Tool_contracts.RollbackAgentSendPreflightFacts.get_agent_id facts
  in
  let run_id =
    Tool_contracts.RollbackAgentSendPreflightFacts.get_run_id facts
  in
  let submission_id =
    Tool_contracts.RollbackAgentSendPreflightFacts.get_submission_id facts
  in
  let previous_submission_id =
    Tool_contracts.RollbackAgentSendPreflightFacts.get_previous_submission_id
      facts
  in
  let outcome =
    match
      Boundary_contracts.RollbackAgentSendPreflightFacts.get_outcome facts
    with
  ...
  let previous_reason_code =
    Option.map
      ...
      (Boundary_contracts.RollbackAgentSendPreflightFacts
       .get_previous_reason_code facts)
  in
  match
    Taumel.Agents.rollback_send_preflight !agent_state
      ~owner_session_id:(owner_id ctx) ~agent_id ~run_id ~submission_id ~outcome
      ~previous_submission_id ~previous_reason_code
  with
  | Error message -> error_obj message
  | Ok next ->
      Session_sync.commit_agent_state ctx next;
...
let record_send_dispatch_failure facts ctx =
  let facts =
    decode_ojs_contract
      Tool_contracts.RecordAgentSendDispatchFailureFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let run_id =
    Tool_contracts.RecordAgentSendDispatchFailureFacts.get_run_id facts
  in
  let submission_id =
    Tool_contracts.RecordAgentSendDispatchFailureFacts.get_submission_id facts
  in
  let error =
    Tool_contracts.RecordAgentSendDispatchFailureFacts.get_error facts
  in
  match
...
let rollback_failed_interruption facts ctx =
  let facts =
    decode_ojs_contract
      Tool_contracts.RollbackFailedAgentInterruptionFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let agent_id =
    Tool_contracts.RollbackFailedAgentInterruptionFacts.get_agent_id facts
  in
  let run_id =
    Tool_contracts.RollbackFailedAgentInterruptionFacts.get_run_id facts
  in
  match
...
let record_dispatch_completion facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.AgentDispatchCompletionFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let run_id = Tool_contracts.AgentDispatchCompletionFacts.get_run_id facts in
  let submission_id =
    Tool_contracts.AgentDispatchCompletionFacts.get_submission_id facts
  in
  let completion =
    Tool_contracts.AgentDispatchCompletionFacts.get_completion facts
  in
  let status =
  ...
  in
  let final_output =
    Tool_contracts.AgentDispatchCompletion.get_finalOutput completion
  in
  let result_entry_id =
    Tool_contracts.AgentDispatchCompletion.get_resultEntryId completion
  in
  ...
  match
    ...
    | Taumel.Agents.Failed ->
        Taumel.Agents.record_run_completion !agent_state ~now ~run_id
          ~status:Failed ?reason_code ?error ?partial_output:final_output
    ...
    | other ->
        Taumel.Agents.record_run_completion !agent_state ~now ~run_id
          ~status:other ?reason_code ?error ?partial_output:final_output
          ?result_entry_id ?submission_id ()
  with
...
let record_activity facts ctx =
  let facts =
    decode_ojs_contract Boundary_contracts.AgentActivityFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let run_id = Tool_contracts.AgentActivityFacts.get_run_id facts in
  let submission_id =
    Tool_contracts.AgentActivityFacts.get_submission_id facts
  in
  let event =
    match Boundary_contracts.AgentActivityFacts.get_event facts with
    | `V_agent_start -> Taumel.Agents.Agent_start
    | `V_turn_start -> Turn_start
    | `V_turn_end -> Turn_end
    | `V_tool_execution_start -> Tool_execution_start
...
let record_dispatch_boundary_result facts ctx =
  ...
  let submission_id =
    Tool_contracts.AgentDispatchBoundaryFacts.get_submission_id facts
  in
  let previous_assistant_entry_id =
    Tool_contracts.AgentDispatchBoundaryFacts.get_previous_assistant_entry_id
      facts
  in
  match
...
let reconcile_live_dispatches facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.LiveAgentDispatchesFacts.t_of_js
      (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  let live_agent_ids =
    Tool_contracts.LiveAgentDispatchesFacts.get_live_agent_ids facts
  in
  let next =
...
let pending_agent_notifications ctx =
  ...
  let pending =
    Taumel.Agent_registry.pending_notifications !agent_state
        ...
        | Some identity ->
            ...
            Some
              (Boundary_contracts.AgentNotification.create ~runId:run.run_id
                 ~content:
                   (Taumel.Agent_registry.completion_message identity run)
                 ~details ()))
      pending
  in
...
let record_background_notification facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.AgentRunIdFacts.t_of_js (ojs_of_js facts)
  in
  Session_sync.require_agent_owner ctx;
  ...
  match
    if owned then
      Taumel.Agent_registry.mark_notification_sent !agent_state ~run_id
...
let count_active_child_runs ctx =
  Session_sync.require_agent_owner ctx;
  let count =
    Taumel.Agent_registry.count_active_child_runs !agent_state
...
let ephemeral_cleanup_plan ctx =
  Session_sync.require_agent_owner ctx;
  let agents =
    Taumel.Agents.owned_identities !agent_state ~owner_session_id:(owner_id ctx)
      ...
      (fun (identity : Taumel.Agents.identity) ->
        ...
        Tool_contracts.AgentManagerIdentity.create
          ...
          ?tier:
            ...
            |> Option.map
                 Boundary_contracts.AgentManagerIdentity.tier_to_contract)
          ())
      identities
  in
  let owned_ids =
    ...
    |> List.map (fun (run : Taumel.Agents.agent_run) ->
        ...
        let recommendation =
          ...
          | ( Taumel.Agents.Running,
              ( Taumel.Agents.Starting | Taumel.Agents.Reasoning
              | Taumel.Agents.Using_tool ) ) ->
              `V_wait
          | Taumel.Agents.Running, Taumel.Agents.Orphaned ->
              `V_interrupt_or_close
          | ( ( Taumel.Agents.Completed | Taumel.Agents.Failed
              | Taumel.Agents.Cancelled | Taumel.Agents.Lost ),
              Taumel.Agents.Inactive ) ->
              `V_call_agent_wait
          | Taumel.Agents.Suspended, Taumel.Agents.Inactive ->
              `V_resume_or_close
        ...
        Tool_contracts.AgentManagerRun.create ~runId:run.run_id
          ...
          ~status:
            (Boundary_contracts.AgentManagerRun.status_to_contract
            ...
            (Option.map
               (fun reason ->
                 Boundary_contracts.AgentManagerRun.reason_code_to_contract
                   (match reason with
                   | Taumel.Agents.Interrupted_by_parent ->
                       `V_interrupted_by_parent
          ...
          ~turnCount:(float_of_int run.run_turn_count)
          ?lastActivityAt:(Option.map float_of_int run.run_last_activity_at)
          ~activityState:activity
          ~recommendation:
            (Boundary_contracts.AgentManagerRun.recommendation_to_contract
               recommendation)
          ~submissionId:run.run_submission_id ?error:run.run_error
          ~announcement:
            (Boundary_contracts.AgentManagerRun.announcement_to_contract
      ...
      | Ok _lease -> (
          ...
          let rec stage_all acc = function
            ...
            | identity :: rest -> (
                ...
                | Ok _ -> (
                    match
                      Agent_child_session_host.authorized_private_session
                        ~identity
                    with
                    | Error message -> Error message
                    | Ok authorized -> (
                        match
                          Agent_child_session_host
                          .stage_authorized_private_session ~ephemeral:true
                            ~identity authorized
                        with
                        | Error message -> (
                            ...
                            | Error unstage_message ->
                                Error
                                 ...
                                 ^ unstage_message))
                        | Ok staged ->
                            stage_all ((identity, staged) :: acc) rest)))
          in
          let rec tombstone state pending_acc = function
            ...
            | (identity, staged) :: rest -> (
                ...
                | None -> (
                    match
                      Taumel.Agent_registry.record_close state
                        ~owner_session_id:owner
          ...
          let finalize_pending state pending_items =
            List.fold_left
              (fun result pending_item ->
                ...
                | Ok current -> (
                    match
                      Agent_child_session_host.finalize_cleanup_pending
                        pending_item
                    with
                    | Error message -> Error message
                    | Ok () ->
                        ignore
                          (Agent_child_session_host
                           .remove_cleanup_journal_record
                             ~owner_session_id:
                               pending_item.cleanup_owner_session_id
                   ...
                   in-process cleanup authority is still available. *)
                    ...
                    | Error message -> (
                        ...
                        | Some journal_message ->
                            error_obj
                              ("cleanup_failed: cleanup journal publication \
                                failed: " ^ journal_message
...
let finish_wait raw_facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.FinishAgentWaitFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  Session_sync.require_agent_owner ctx;
  let run_ids = Tool_contracts.FinishAgentWaitFacts.get_run_ids facts in
  match
    Taumel.Agent_wait.wait_for_run_ids !agent_state
      ~owner_session_id:(owner_id ctx) run_ids
  with
  | Error message -> error_obj message
  | Ok wait ->
...
let agent_runs_summary owner_id =
  match
    Taumel.Agent_registry.list_for_owner !agent_state ~owner_session_id:owner_id
  with
  | [] -> "No agents."
  | items ->
      String.concat "\n"
        (List.map
           (fun (identity, latest) -> agent_identity_line identity latest)
           items)
...
let stop_active_run state ~now ~owner_session_id ~agent_id =
  ...
  | Ok _ -> (
      ...
      | Some run when run.run_status = Taumel.Agents.Running -> (
          ...
          with
          | Error _ as error -> error
          | Ok delivery -> Ok (delivery.delivery_state, true, run.run_id))
  ...
  | None -> (
      ...
      | "stop" -> (
          ...
          if target = "" then error_obj "usage: /agent-runs stop <agent-id>"
            ...
            match
              stop_active_run !agent_state ~now:(now_seconds ())
                ~owner_session_id:owner ~agent_id:target
            with
            | Error message -> error_obj message
                          ...
                          (if changed then
                             [
                               Tool_contracts.AgentChildSessionUpdate.create
                                 ~action:
                                   (Boundary_contracts.AgentChildSessionUpdate
                                    .action_to_contract `V_stop_child_session)
      ...
      | "close" -> error_obj "agent close requires manager confirmation"
      | "output" -> (
          let target = String.trim rest in
          if target = "" then
            error_obj "usage: /agent-runs output <agent-id|run-id>"
          else
            let run =
              match Taumel.Agents.find_run !agent_state target with
              | Some run -> (
                  match
                    Taumel.Agents.find_identity !agent_state run.run_agent_id
                  with
                  | Some identity
                    when identity.identity_owner_session_id = owner ->
                      Ok run
                  | _ -> Error ("run is not owned by this session: " ^ target))
              | None -> (
                  match
                    Taumel.Agents.owned_identity !agent_state
                      ~owner_session_id:owner target
                  with
                  | Error _ as error -> error
                  | Ok _ -> (
            ...
            | Ok selected_run ->
                ...
                let text, truncated, path =
                  truncate_output ~owner_session_id:owner
                    ~agent_id:run.run_agent_id ~run_id:run.run_id output
                in
                let details =
                  Unsafe.obj
                    [|
      ...
      | "instruction" -> (
          let target = String.trim rest in
          if target = "" then
            error_obj "usage: /agent-runs instruction <agent-id>"
          else
            match
      ...
      | _ ->
          error_obj
            "usage: /agent-runs [list|stop <agent-id>|close <agent-id>|output \
             <agent-id|run-id>|instruction <agent-id>]")
