...
let opt_string_default default = function
  | Some value -> value
  | None -> default
...
let resolve_authorization_path path =
  try
    match
      string_value
        (call1 (active_host_or_empty ()) "resolveAuthorizationPath"
           (js_string path))
    with
  ...
  with error ->
    Error
      ("path authorization failed for " ^ path ^ ": " ^ Printexc.to_string error)
...
let exec_request_from_params params =
  let params =
    decode_ojs_contract Tool_contracts.ExecCommandParams.t_of_js
      (ojs_of_js params)
  in
  let sandbox_permissions =
    match
      Tool_contracts.ExecCommandParams.get_with_escalated_permissions params
    with
    | Some true ->
        let justification =
  ...
  in
  ({
     Taumel.Mutation_plan.cmd = Tool_contracts.ExecCommandParams.get_cmd params;
     workdir =
       opt_string_default ""
...
let write_stdin_request_from_params params =
  let params =
    decode_ojs_contract Tool_contracts.WriteStdinParams.t_of_js
      (ojs_of_js params)
  in
  ({
...
let write_request_from_params params =
  let params =
    decode_ojs_contract Tool_contracts.WriteParams.t_of_js (ojs_of_js params)
  in
  ({
...
let edit_request_from_params params =
  let params =
    decode_ojs_contract Tool_contracts.EditParams.t_of_js (ojs_of_js params)
  in
  ({
...
let patch_request_from_params params =
  let params =
    decode_ojs_contract Tool_contracts.ApplyPatchParams.t_of_js
      (ojs_of_js params)
  in
...
let resolve_trusted_git () = Agent_worktree_host.trusted_git_executable ()

let parse_brokered_git cmd =
  match Exec_policy_bridge.reflect_bash_script cmd with
  | Error _ ->
      Error
        (Taumel.Agent_git_broker.error_message
           Taumel.Agent_git_broker.Not_simple_git)
  | Ok ast -> (
  ...
  | Some worktree -> (
      ...
      | Ok root ->
          if List.for_all (Taumel.Sandbox.path_within ~root) paths then Ok ()
            ...
            Error
              "worktree-isolated child mutation must stay inside its worktree")
              ...
              | Some _, true, Some Taumel.Exec_policy.Forbidden ->
                  error_obj
                    (Option.value policy_message
                       ~default:"exec policy forbids command")
              | Some _, true, Some Taumel.Exec_policy.Prompt ->
                  error_obj
                    (Option.value policy_message
                       ~default:
                         "exec policy requires approval before brokered agent \
                          Git")
              | _ -> (
                  let brokered =
                    ...
                    | Some worktree_context ->
                        ...
                        | Ok parsed ->
                            ...
                            | Taumel.Sandbox.Use_default ->
                                ...
                                in
                                begin match
                                  Taumel.Agent_git_broker.authorize ~read_only
                                    parsed
                                with
                                | Error error ->
                                    Error
                                ...
                                | Ok authorized ->
                                    let agent_id =
                                      Option.value
                                        (child_agent_id child_metadata)
                                        ~default:""
                                    in
                                    if
                                      agent_id <> ""
                                      && Taumel.Agent_git_broker.Lease.is_held
                                           agent_id
                                    then
                                      Error
                                        "brokered agent Git is already running \
                                         for this identity"
                                    else
                                      begin match
                                        Agent_worktree_host
                                        .verify_broker_registration
                                          ...
                                          ~main_repository_id:
                                            worktree_context.main_repository_id
                                          ~branch
                                      with
                                      | Error message -> Error message
                                      | Ok git_dir ->
                  ...
                  | Ok (worktree, git_dir, argv, agent_id, subcommand) -> (
                      ...
                      let workdir =
                        if request.workdir = "" then worktree
                          ...
                          || Taumel.Sandbox.path_within ~root:worktree
                               request.workdir
                      ...
                      then
                        error_obj
                          "brokered agent Git workdir must stay inside the \
                           agent worktree"
                      else
                        ...
                        | Ok trusted_git -> (
                            ...
                            | Ok (host, shell) ->
                                let subcommand =
                                  Taumel.Agent_git_broker.subcommand_to_string
                                    subcommand
                                in
                                let plan =
                                  {
                                ...
                                let planId =
                                  Authority_plans.issue_exec
                                    ~owner_id:
                                      (Session_store.session_id_from_ctx ctx)
                                    ~owner_context:ctx ~approval_required:false
                                    plan
                                in
                                Boundary_contracts.PreparedExec.create ~planId
                                  ~cmd:request.cmd ~workdir ~tty:false
                                  ~sandbox:sandbox_cfg ~brokeredGit:true ()
                  ...
                  | Error _ -> (
                      match
                        Taumel.Mutation_plan.plan_exec ?policy_decision
                          ?policy_message sandbox request
                      with
                      | Error message -> error_obj message
                      | Ok plan -> (
                          let sandbox_cfg = typed_sandbox_config sandbox in
                          ...
                          let maxOutputTokens =
                            Option.map float_of_int plan.max_output_tokens
                          in
                                  ...
                                  let planId =
                                    Authority_plans.issue_exec
                                      ~owner_id:
                                        (Session_store.session_id_from_ctx ctx)
                                      ~owner_context:ctx
                                      ~approval_required:false authority_plan
                                  in
                                  Boundary_contracts.PreparedExec.create ~planId
                                    ~cmd:plan.cmd ~workdir:plan.workdir
                                    ?yieldTimeMs ?maxOutputTokens ~tty:plan.tty
                                    ~sandbox:sandbox_cfg ()
                                  |> Tool_contracts.PreparedExec.t_to_js
                                  |> inject
                              | Some approval ->
                                  let execPolicyAllowAlwaysTokens =
                                    if
                                      not
                                        (String.starts_with
                                           ~prefix:
                                             "exec policy requires approval"
                                           approval.message)
                                    then None
                                    else if
                                      Exec_policy_bridge
                                      .explicit_prompt_or_forbidden request.cmd
                                    then None
                                    else
                                      Exec_policy_bridge.allow_amendment_tokens
                                        request.cmd
                                  in
                                  let planId =
                                    Authority_plans.issue_exec
                                      ...
                                      authority_plan
                                  in
                                  Boundary_contracts.PreparedExecApproval.create
                                    ~planId ~cmd:plan.cmd ~workdir:plan.workdir
                                    ?yieldTimeMs ?maxOutputTokens ~tty:plan.tty
                                    ~sandbox:sandbox_cfg
                                    ...
                                    ~approvalTimeoutMs:
                                      (float_of_int approval.timeout_ms)
                                    ?execPolicyAllowAlwaysTokens ()
                                  |> Tool_contracts.PreparedExecApproval.t_to_js
                                  |> inject))))))
  ...
  with_gateway_authorized "write_stdin" (fun _sandbox ->
      ...
      | Ok plan ->
          ...
          Boundary_contracts.PreparedWriteStdin.create
            ~sessionId:(float_of_int plan.session_id)
            ~chars:plan.chars ?yieldTimeMs:plan.yield_time_ms
...
let prepared_write_mode = function
  | "overwrite" ->
      Boundary_contracts.PreparedWrite.mode_to_contract `V_overwrite
...
let prepare_write params ctx =
  with_child_session_metadata ctx (fun child_metadata ->
      with_gateway_profile_authorized "write" (fun sandbox ->
          ...
          match
            path_authorization_for_child child_metadata sandbox request.path
          with
          | Error message -> error_obj message
          | Ok (auth_path, auth_roots) -> (
              match
              ...
              | Error message -> error_obj message
              | Ok plan
                when child_rejects_filesystem_approval child_metadata
                       plan.approval ->
                  error_obj
                    "worktree-isolated child filesystem approval is forbidden"
              | Ok plan -> (
...
let prepare_read params =
  with_gateway_authorized "read" (fun _sandbox ->
      let params =
        decode_ojs_contract Tool_contracts.ReadParams.t_of_js (ojs_of_js params)
      in
...
let prepare_edit params ctx =
  with_child_session_metadata ctx (fun child_metadata ->
      with_gateway_profile_authorized "edit" (fun sandbox ->
          ...
          | Ok (auth_path, auth_roots) -> (
              match
                Taumel.Mutation_plan.plan_edit ~auth_path ~auth_roots sandbox
                  request
              with
              | Error message -> error_obj message
              | Ok plan
...
let apply_edit_to_file raw_facts =
  let facts =
    decode_ojs_contract Tool_contracts.EditApplicationFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  ...
  let display_path =
    Tool_contracts.EditApplicationFacts.get_displayPath facts
  in
  let edits =
  ...
  | Ok contents ->
      Boundary_contracts.EditApplied.create ~path ~displayPath:display_path
        ~contents
        ~editCount:(float_of_int (List.length edits))
        ()
...
let prepare_apply_patch params ctx =
  with_child_session_metadata ctx (fun child_metadata ->
      with_gateway_profile_authorized "apply_patch" (fun sandbox ->
          ...
          | Ok request -> (
              match
                patch_authorization_for_child child_metadata sandbox
                  request.patch
              with
              | Error message -> error_obj message
              | Ok (auth_paths, auth_roots) -> (
                  ...
                  in
                  match
                    Taumel.Mutation_plan.plan_apply_patch ~auth_paths
                      ~auth_roots sandbox request
                  with
                  | Error message -> error_obj message
                  | Ok plan
                    when child_rejects_filesystem_approval child_metadata
                           plan.approval ->
                      error_obj
                        "worktree-isolated child filesystem approval is \
                         forbidden"
                  | Ok plan -> (
                      match plan.approval with
                      | None ->
                      ...
                      | Some approval ->
                          Boundary_contracts.PreparedPatchApproval.create
                            ...
                            ~affectedPaths:plan.affected_paths
                            ~authorizedPaths:authorized_paths
                            ~patch:request.patch ~approvalTitle:approval.title
                            ~approvalPrompt:approval.prompt
                            ~approvalTimeoutMs:
                              (float_of_int approval.timeout_ms)
                            ()
                          |> Tool_contracts.PreparedPatchApproval.t_to_js
                          |> inject)))))
...
let apply_patch_to_files raw_facts =
  let facts =
    decode_ojs_contract Tool_contracts.PatchApplicationFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  let params = Tool_contracts.PatchApplicationFacts.get_params facts in
  let files =
    Tool_contracts.PatchApplicationFacts.get_files facts
    |> Ts2ocaml.unknown_to_js |> js_of_ojs
  in
  let ctx =
    Tool_contracts.PatchApplicationFacts.get_ctx facts
    |> Ts2ocaml.unknown_to_js |> js_of_ojs
  in
  Session_sync.require_session_from_host ~scope:"apply_patch files" ctx;
  let approved =
    Tool_contracts.PatchApplicationFacts.get_filesystemApproval facts
  in
  let authorized_paths =
  ...
  | Ok child_metadata ->
      with_gateway_profile_authorized "apply_patch" (fun sandbox ->
          ...
          | Ok request -> (
              match
                validate_frozen_patch_authorizations request.patch
                  authorized_paths
              with
              | Error message -> mutation_error message
              | Ok () -> (
                  ...
                  | Ok (current_auth_paths, auth_roots)
                    when List.sort compare current_auth_paths
                      ...
                      mutation_error
                        "patch authorization paths changed before application"
                  | Ok (_current_auth_paths, auth_roots) -> (
                      match
                        Taumel.Mutation_plan.apply_patch_to_files ~approved
                          ~auth_paths:authorized_paths ~auth_roots sandbox
                          request (files_map_from_js files)
                      with
                      | Error message -> mutation_error message
                      | Ok output ->
                          let write_objects =
