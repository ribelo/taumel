...
type candidate = { data : string; mime_type : string; encoded_size : int }

let prepare params =
  with_gateway_authorized "view_media" (fun _sandbox ->
      let params =
        decode_ojs_contract Tool_contracts.ViewMediaParams.t_of_js
          (ojs_of_js params)
      in
      let path = Tool_contracts.ViewMediaParams.get_path params in
      if String.trim path = "" then
        error_obj "view_media requires a non-empty path"
      else
  ...
  if len < 24 then None
    ...
    let width =
      if byte_at bytes 16 <> 0 then max_pixel_count + 1
        ...
        int24_le_bytes bytes 17 |> fun value ->
        (value land 0xff * 65536)
        + (value land 0xff00)
    ...
    let height =
      if byte_at bytes 20 <> 0 then max_pixel_count + 1
        ...
        int24_le_bytes bytes 21 |> fun value ->
        (value land 0xff * 65536)
        + (value land 0xff00)
        + ((value lsr 16) land 0xff)
    in
  ...
  let rec scan pos =
    if pos + 9 > len then None
      ...
      | _ ->
          ...
          if segment_len < 2 then None else scan (pos + 2 + segment_len)
  in
  ...
  if len < 30 then None
    ...
    | "VP8 " ->
        if len < 30 then None
          ...
          let width =
            (byte_at bytes 26 + (byte_at bytes 27 lsl 8)) land 0x3fff
          in
          let height =
            (byte_at bytes 28 + (byte_at bytes 29 lsl 8)) land 0x3fff
          in
          if width > 0 && height > 0 then Some (width, height) else None
    | "VP8L" ->
        if len < 25 then None
    ...
    | "VP8X" ->
        if len < 30 then None
          ...
          let width =
            byte_at bytes 24
            + (byte_at bytes 25 lsl 8)
            + (byte_at bytes 26 lsl 16)
            + 1
          in
          let height =
            byte_at bytes 27
            + (byte_at bytes 28 lsl 8)
            + (byte_at bytes 29 lsl 16)
            + 1
          in
          if width > 0 && height > 0 then Some (width, height) else None
    | _ -> None
  ...
  if pos + 8 > len then false
    ...
    if tag = "ANIM" then true
      ...
      if high_byte <> 0 then false
        ...
        let padded_size =
          if chunk_size land 1 = 1 then chunk_size + 1 else chunk_size
        in
...
let is_animated_gif bytes =
  ...
  if len < 13 then false
    ...
    let gct_bytes =
      if packed land 0x80 = 0 then 0 else 3 * (1 lsl ((packed land 0x07) + 1))
    in
...
let try_encodings photon image width height =
  let sampling_filter =
    Unsafe.get (Unsafe.get photon "SamplingFilter") "Lanczos3"
  in
  let resized =
    Unsafe.fun_call
      (Unsafe.get photon "resize")
      [|
        image;
  ...
  try
    let png =
      encode_candidate (Unsafe.meth_call resized "get_bytes" [||]) "image/png"
    in
    let jpeg_candidates =
...
let resize_image photon image input_bytes mime_type =
  ...
  then
    ...
    let start_width, start_height =
      fit_dimensions original_width original_height
    in
    let rec loop width height =
      match best_under_limit (try_encodings photon image width height) with
      | Some candidate ->
          Ok (candidate, original_width, original_height, width, height, true)
      | None ->
          if width = 1 && height = 1 then
            ...
            let next_height =
              if height = 1 then 1 else max 1 (height * 3 / 4)
            in
...
let success_envelope text details candidate =
  let text_content =
    ...
    |> Tool_contracts.ToolResultTextContent.t_to_js |> Ts2ocaml.unknown_of_js
  in
  let image_content =
    Boundary_contracts.ToolResultImageContent.create ~data:candidate.data
      ~mimeType:candidate.mime_type ()
    |> Tool_contracts.ToolResultImageContent.t_to_js |> Ts2ocaml.unknown_of_js
  in
...
let success_result ~path ~full_path ~original_mime_type candidate
    ~original_width ~original_height ~width ~height ~was_resized ~byte_length =
  let dimension_text =
    if was_resized then
      Printf.sprintf "%dx%d -> %dx%d" original_width original_height width
        height
    ...
    | Ok stat -> (
        if not (Node_fs.is_file stat) then
          ...
          if source_size_float > float_of_int max_source_bytes then
            error_result ~path ~full_path
              (Printf.sprintf "image at `%s` exceeds the 64 MiB safety ceiling"
                 full_path)
          else
            ...
            | Error error ->
                error_result ~path ~full_path
                  (Printf.sprintf "unable to read image at `%s`: %s" full_path
                     error)
            | Ok input_bytes -> (
                ...
                | None ->
                    error_result ~path ~full_path
                      (Printf.sprintf
                         "unsupported image format at `%s`; supported formats \
                ...
                | Some mime_type -> (
                    ...
                    | None ->
                        error_result ~path ~full_path
                          (Printf.sprintf
                             "could not determine image dimensions at `%s`; \
                    ...
                    | Some (decoded_width, decoded_height) -> (
                        ...
                        if exceeds_pixel_limit then
                          error_result ~path ~full_path
                            (Printf.sprintf
                               "image at `%s` exceeds the 64 megapixel safety \
                                ceiling"
                               full_path)
                        else
                          ...
                          let animated =
                            ...
                            | _ -> false
                          in
                          let input_base64_encoded_size =
                            base64_size source_size
                          in
                          if
                            animated && decoded_width <= max_width
                          ...
                          then
                            ...
                            error_result ~path ~full_path
                              (Printf.sprintf
                                 "animated image at `%s` would require \
                                  resizing or re-encoding, which is not \
                                  supported; provide a smaller animation or a \
                                  still image"
                                 full_path)
                          else
                            ...
                            | Error error ->
                                error_result ~path ~full_path
                                  ("unable to load @silvia-odwyer/photon-node: "
                                 ^ error)
                            | Ok photon -> (
                                let image_result =
                                  try
                                ...
                                | Error error ->
                                    error_result ~path ~full_path
                                      (Printf.sprintf
                                         "image at `%s` is invalid: %s"
                                         full_path error)
                                | Ok image -> (
                                    let result =
                                      try
                                        resize_image photon image input_bytes
                                          mime_type
                                      with exn ->
                                    ...
                                    match result with
                                    | Error error ->
                                        error_result ~path ~full_path error
                                    | Ok
                                        ( candidate,
                                          original_width,
                                          original_height,
