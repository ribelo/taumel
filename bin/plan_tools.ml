...
let js_plan (plan : Taumel.Plan.t) =
  Unsafe.obj
    [|
      ...
      ("completedTasks", js_number (float_of_int (Taumel.Plan.completed_task_count plan.tasks)));
       ...
       [
         ...
         ( "automation",
           Taumel.Shared.Object
             [
               ( "continuation",
                 Taumel.Shared.String (Taumel.Plan.automation_to_string automation) );
               ( "requiresUserInput",
                 Taumel.Shared.Bool
...
let command_result ?(followup = false) ?(inspection = false) ?submit_user_message
    ?rollback plan message =
  let details = Ts2ocaml.unknown_of_js (ojs_of_js (details plan !plan_automation)) in
  ...
  Boundary_contracts.GatewayCommandResult.create ~ok:true ~message ~details
    ?planFollowup ?planSubmitUserMessage:submit_user_message ?planRollback ?planInspection ()
...
let continuation_facts facts =
  {
    ...
    has_pending_messages = Tool_contracts.PlanContinuationFacts.get_hasPendingMessages facts;
    ...
    latest_assistant_stop_reason = Tool_contracts.PlanContinuationFacts.get_latestAssistantStopReason facts;
  }

let plan_continuation raw_facts =
  let facts = decode_ojs_contract Tool_contracts.PlanContinuationFacts.t_of_js (ojs_of_js raw_facts) in
  let ctx =
  ...
  in
  match Session_sync.try_sync_session_from_host ~scope:"plan continuation" ctx with
  | Error _ -> none ()
  | Ok () -> (
      match
      ...
      | Taumel.Plan.Send_continuation continuation ->
          Boundary_contracts.PlanContinuationSend.create
            ...
            ~details:
              (decode_ojs_contract Tool_contracts.PlanPresentationDetails.t_of_js
...
let plan_store_of_js value =
  ...
  | Ok json -> (match Taumel.Plan.codec.decode json with Ok store -> store | Error _ -> None)
  | Error _ -> None

let automation_of_js value =
  match json_from_js value with
  | Ok json -> (match Taumel.Plan.automation_codec.decode json with
      | Ok automation -> automation | Error _ -> Taumel.Plan.Automation_enabled)
...
let plan_child_plan_continuation facts =
  ...
  | Taumel.Plan.Child_finalize { child_status; child_reason } ->
      Boundary_contracts.ChildPlanContinuationFinalize.create ~status:child_status
        ?reason:child_reason ()
...
let task_creation (item : Tool_contracts.CreateTaskItem.t) : Taumel.Plan.task_creation =
  {
    ...
    depends_on = Option.value ~default:[] (Tool_contracts.CreateTaskItem.get_depends_on item);
  }
...
let prepare_update_task params ctx =
  with_gateway_authorized "update_task" (fun _ ->
      let params = decode_ojs_contract Tool_contracts.UpdateTaskParams.t_of_js (ojs_of_js params) in
      let update : Taumel.Plan.task_update =
        {
          title = Tool_contracts.UpdateTaskParams.get_title params;
          description = (match Tool_contracts.UpdateTaskParams.get_description params with
            | None -> Taumel.Plan.Keep_description
            | Some value -> Taumel.Plan.Set_description value);
          status = Option.map task_status (Boundary_contracts.UpdateTaskParams.get_status params);
          depends_on = Tool_contracts.UpdateTaskParams.get_depends_on params;
        }
      in
      match Taumel.Plan.update_task ~now:(now_seconds ())
              ~task_id:(Tool_contracts.UpdateTaskParams.get_taskId params)
              update !current_plan with
      | Error message -> error_obj message
      | Ok plan ->
          current_plan := Some plan;
          Session_sync.save_plan_state ctx;
...
let prepare_update_plan params ctx =
  with_gateway_authorized "update_plan" (fun _ ->
      let params = decode_ojs_contract Tool_contracts.UpdatePlanParams.t_of_js (ojs_of_js params) in
      let status, pending =
        match Boundary_contracts.UpdatePlanParams.get_status params with
        | `V_active -> (Taumel.Plan.Active, None)
        | `V_complete -> (Taumel.Plan.Complete, Some Taumel.Plan.Pending_complete)
        | `V_blocked -> (Taumel.Plan.Blocked, Some Taumel.Plan.Pending_blocked)
      in
      match Taumel.Plan.update_plan ~now:(now_seconds ()) status !current_plan with
      | Error message -> error_obj message
      | Ok plan ->
          (match pending with
...
let create_from_cron raw_facts =
  let facts = decode_ojs_contract Tool_contracts.CronPlanCreationFacts.t_of_js (ojs_of_js raw_facts) in
  let title = Tool_contracts.CronPlanCreationFacts.get_title facts in
  let ctx = Tool_contracts.CronPlanCreationFacts.get_ctx facts |> Ts2ocaml.unknown_to_js |> js_of_ojs in
  let store =
    match !current_plan with
    | Some plan when plan.status = Taumel.Plan.Complete -> None
    | store -> store
  in
  (match Taumel.Plan.add_user_task ~session_id:(session_id ctx) ~now:(now_seconds ()) title store with
...
let finalize_error _status ctx =
  let next = Taumel.Plan.final_unrecoverable_error ~now:(now_seconds ()) !current_plan in
  if next <> !current_plan then (current_plan := next; Session_sync.save_plan_state ctx)

let handle_command args ctx =
  let previous_plan = !current_plan in
  let previous_automation = !plan_automation in
  match Taumel.Plan.apply_command ~automation:!plan_automation
          ~session_id:(session_id ctx) ~now:(now_seconds ()) args !current_plan with
  | Error message -> error_obj message
  | Ok result ->
      if result.changed then (
        pending_plan_terminal_status := None;
        current_plan := result.plan;
        Option.iter (fun automation -> plan_automation := automation) result.automation;
        Session_sync.save_plan_state ctx;
        if Option.is_some result.automation then Session_sync.save_plan_automation_state ctx);
      let rollback =
        match result.submit_user_message with
        | None -> None
        | Some _ ->
            let plan = match previous_plan with None -> Unsafe.inject Js.null | Some value -> inject (js_plan value) in
            Some (Unsafe.obj [| ("plan", plan); ("automation", inject (js_automation previous_automation)) |])
      in
      command_result ~followup:result.followup ~inspection:(String.trim args = "")
        ?submit_user_message:result.submit_user_message ?rollback result.plan result.message

let rollback_plan_command raw_facts =
  let facts = decode_ojs_contract Tool_contracts.PlanRollbackFacts.t_of_js (ojs_of_js raw_facts) in
  let snapshot = Tool_contracts.PlanRollbackFacts.get_snapshot facts |> Ts2ocaml.unknown_to_js |> js_of_ojs in
  let ctx = Tool_contracts.PlanRollbackFacts.get_ctx facts |> Ts2ocaml.unknown_to_js |> js_of_ojs in
