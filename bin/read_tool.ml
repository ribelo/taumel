...
let read_file raw_facts =
  let facts =
    decode_ojs_contract Tool_contracts.ReadFileFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  let path = Tool_contracts.ReadFileFacts.get_path facts in
  let cwd = Tool_contracts.ReadFileFacts.get_defaultCwd facts in
  let offset =
    Tool_contracts.ReadFileFacts.get_offset facts |> Option.map int_of_float
  in
  let limit =
    Tool_contracts.ReadFileFacts.get_limit facts |> Option.map int_of_float
  in
  if String.trim path = "" then error_result "read requires a non-empty path"
    ...
    | None ->
        if is_symbolic_link resolved then
          error_result
            (Printf.sprintf
               "\"%s\" is a symbolic link whose target does not exist or \
                cannot be accessed."
               path)
    ...
    | Some stat -> (
        if Node_fs.is_directory stat then
          error_result
            (Printf.sprintf
               "\"%s\" is not a file. Use exec_command (e.g. `ls`) to inspect \
                a directory."
               path)
        else
          let content =
          ...
          | None ->
              error_result
                (Printf.sprintf "Could not read \"%s\" as text." path)
          | Some content -> (
              match Taumel.File_read.format ~content ~offset ~limit with
              | Taumel.File_read.Binary_content ->
                  error_result
                    (Printf.sprintf
                       "\"%s\" is not readable as UTF-8 text. If it is an \
                        image or other binary file, inspect it with \
                        exec_command (e.g. `file`, `xxd`)."
                       path)
