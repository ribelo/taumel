...
let issue_agent_action ~owner_id ~owner_context ~action ~agent_id ~owner_epoch
  ...
  let state_epoch = Agent_state_epochs.advance ~owner_id ~agent_id in
  let id = fresh_id () in
...
let validate_agent_action ~owner_id ~action ~agent_id ~owner_epoch
  ...
  | Some entry when now_ms > entry.expires_at_ms ->
      ignore
        (Agent_state_epochs.advance ~owner_id:entry.owner_id
...
let claim_agent_action ~owner_id ~action ~agent_id ~owner_epoch
  ...
  | Ok entry ->
      ...
      entry.state_epoch <- Agent_state_epochs.advance ~owner_id ~agent_id;
      Ok ()
...
let ratchet_agent_action_state ~owner_id ~action ~agent_id ~owner_epoch
    ~permission_epoch ~now_ms id =
  match Hashtbl.find_opt agent_action_entries id with
  | None -> Error invalid_plan
  ...
  | Some entry ->
      ...
      if entry.state_epoch = current then (
        entry.state_epoch <- Agent_state_epochs.advance ~owner_id ~agent_id;
        Ok ())
      else Error "agent action capability is stale"

let adopt_agent_action_transition ~owner_id ~action ~agent_id ~owner_epoch
    ~permission_epoch ~now_ms id =
  match Hashtbl.find_opt agent_action_entries id with
  | None -> Error invalid_plan
  | Some entry when entry.owner_id <> owner_id -> Error wrong_owner
  ...
  | Some entry ->
      ...
      if entry.state_epoch + 1 = current then (
        entry.state_epoch <- current;
        Ok ())
      else Error "agent action capability transition is not contiguous"

let complete_agent_action_transition ~owner_id ~action ~agent_id ~owner_epoch
    ~permission_epoch ~now_ms id =
  match Hashtbl.find_opt agent_action_entries id with
  | Some entry
    when entry.state_epoch
         = Agent_state_epochs.current ~owner_id:entry.owner_id
             ~agent_id:entry.agent_id ->
      ratchet_agent_action_state ~owner_id ~action ~agent_id ~owner_epoch
        ~permission_epoch ~now_ms id
  | _ ->
      adopt_agent_action_transition ~owner_id ~action ~agent_id ~owner_epoch
        ~permission_epoch ~now_ms id
...
let discard_owner owner_id =
  ...
  Agent_state_epochs.discard_owner owner_id
