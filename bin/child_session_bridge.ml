...
let plan_permission_refresh parent_permissions_js metadata_js parent_ctx =
  ...
  let permissions =
    (match
       Session_sync.try_sync_session_from_host ~scope:"child permission refresh"
         parent_ctx
     with
      ...
      | Ok () ->
          Taumel.Child_session.refresh_permissions_entry
            ...
            ~host_no_sandbox:!App_state.host_no_sandbox ~parent_permissions
            metadata)
    ...
    Some
      {
        Taumel.Child_session.session_id =
          Option.bind
            (optional_string_field facts "sessionId")
            Taumel.Shared.trim_non_empty;
        session_file =
          Option.bind
            (optional_string_field facts "sessionFile")
            Taumel.Shared.trim_non_empty;
        cancelled = get_bool facts "cancelled";
        error =
          (match
             Option.bind
               (optional_string_field facts "error")
               Taumel.Shared.trim_non_empty
           with
        ...
        model_id =
          Option.bind
            (optional_string_field facts "modelId")
            Taumel.Shared.trim_non_empty;
        model_applied = get_bool facts "modelApplied";
        thinking_level =
          Option.bind
            (optional_string_field facts "thinkingLevel")
            Taumel.Shared.trim_non_empty;
...
let child_bridge_details facts =
  json_to_js
    (Taumel.Child_session.bridge_details (child_session_bridge_from_js facts))

let plan_child_dispatch facts =
  let facts =
    decode_ojs_contract Tool_contracts.ChildDispatchFacts.t_of_js
      (ojs_of_js facts)
  in
  let bridge =
      ...
      Some
        {
          Taumel.Child_session.session_id =
            Option.bind
              (Tool_contracts.ChildDispatchFacts.get_sessionId facts)
              Taumel.Shared.trim_non_empty;
          session_file =
            Option.bind
              (Tool_contracts.ChildDispatchFacts.get_sessionFile facts)
              Taumel.Shared.trim_non_empty;
          cancelled =
            Option.value
              (Tool_contracts.ChildDispatchFacts.get_cancelled facts)
              ~default:false;
          error =
            (match
               Option.bind
                 (Tool_contracts.ChildDispatchFacts.get_error facts)
                 Taumel.Shared.trim_non_empty
             with
            | Some error -> Some error
            | None
              when Option.value
                     (Tool_contracts.ChildDispatchFacts
                      .get_missingSessionIdentifier facts)
                     ~default:false ->
                Some Taumel.Child_session.missing_session_identifier_error
            | None -> None);
          active_tools = Tool_contracts.ChildDispatchFacts.get_activeTools facts;
          active_tools_applied =
            Option.value
              (Tool_contracts.ChildDispatchFacts.get_activeToolsApplied facts)
              ~default:false;
          model_id =
            Option.bind
              (Tool_contracts.ChildDispatchFacts.get_modelId facts)
              Taumel.Shared.trim_non_empty;
          model_applied =
            Option.value
              (Tool_contracts.ChildDispatchFacts.get_modelApplied facts)
              ~default:false;
          thinking_level =
            Option.bind
              (Tool_contracts.ChildDispatchFacts.get_thinkingLevel facts)
              Taumel.Shared.trim_non_empty;
          thinking_applied =
            Option.value
              (Tool_contracts.ChildDispatchFacts.get_thinkingApplied facts)
              ~default:false;
        }
  in
  let empty_reason =
    Option.value
      (Option.bind
         (Some (Tool_contracts.ChildDispatchFacts.get_emptyReason facts))
         Taumel.Shared.trim_non_empty)
      ~default:"empty prompt"
  in
  let deliver_as =
    Option.bind
      (Tool_contracts.ChildDispatchFacts.get_deliverAs facts)
      Taumel.Shared.trim_non_empty
  in
  Taumel.Child_session.dispatch_plan ?bridge ~empty_reason ?deliver_as
    ~prompt:(Tool_contracts.ChildDispatchFacts.get_prompt facts)
  ...
  Tool_contracts.ChildDispatchPlan.create ~send:plan.send ~prompt:plan.prompt
    ...
    ~result:
      (decode_ojs_contract Tool_contracts.ChildDispatchResult.t_of_js
         (ojs_of_js (json_to_js plan.result)))
    ()
  |> Tool_contracts.ChildDispatchPlan.t_to_js |> inject

let plan_child_session_start raw_facts ctx =
  let facts =
    decode_ojs_contract Tool_contracts.ChildSessionStartFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  let metadata_js =
  ...
  let parent_session_file =
    Tool_contracts.ChildSessionStartFacts.get_parentSessionFile facts
  in
  let plan =
  ...
  let result =
    let privateSessionDirectory =
      match
        Option.bind
          (optional_string_field metadata_js "agentId")
          Taumel.Shared.trim_non_empty
      with
      | None -> None
      | Some agent_id -> (
          ...
          | Error message -> failwith message)
    in
    Tool_contracts.ChildSessionStartPlan.create
      ?parentSession:plan.parent_session ?modelId:plan.model_id
      ?thinkingLevel:plan.thinking_level ?activeTools:plan.active_tools
