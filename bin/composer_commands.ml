...
let handle raw_facts =
  let facts =
    decode_ojs_contract Tool_contracts.ComposerCommandFacts.t_of_js
      (ojs_of_js raw_facts)
  in
  let args = Tool_contracts.ComposerCommandFacts.get_args facts in
  let settings_js =
    Tool_contracts.ComposerCommandFacts.get_settings facts
  ...
  | Ok result ->
      let settings =
        Taumel.Global_settings.to_json result.settings
