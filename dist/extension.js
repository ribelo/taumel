...
__export(exports_bridge_core_contracts, {
  ...
  SkillMessageSchema: () => SkillMessageSchema,
  SkillMessageDetailsSchema: () => SkillMessageDetailsSchema,
  SkillListResultSchema: () => SkillListResultSchema,
  SkillListFactsSchema: () => SkillListFactsSchema,
  SkillInfoSchema: () => SkillInfoSchema,
  SkillExpansionFactsSchema: () => SkillExpansionFactsSchema,
  SkillExpansionEffectsSchema: () => SkillExpansionEffectsSchema,
...
var SkillExpansionFactsSchema = Type7.Object({ text: Type7.String(), cwd: Type7.String(), ctx: Type7.Optional(Type7.Unknown()) }, { $id: "SkillExpansionFacts", additionalProperties: false });
var SkillMessageDetailsSchema = Type7.Object({
  source: Type7.Literal("auto-skill-mention"),
  trigger: Type7.String({ minLength: 1 }),
  name: Type7.String({ minLength: 1 }),
  parent: Type7.Optional(Type7.String({ minLength: 1 }))
}, { $id: "SkillMessageDetails", additionalProperties: false });
var SkillMessageSchema = Type7.Object({
  customType: Type7.Literal("skill"),
  content: Type7.String({ minLength: 1 }),
  display: Type7.Literal(true),
...
var SkillExpansionEffectsSchema = Type7.Object({
  messages: Type7.Array(SkillMessageSchema),
  warnings: Type7.Array(BridgeWarningSchema)
...
var skillExpansionEffectsDecoder = Compile2(SkillExpansionEffectsSchema);
...
function decodeSkillExpansionEffects(value) {
  try {
    return skillExpansionEffectsDecoder.Decode(value);
  } catch {
    throw new Error("Invalid OCaml skill-expansion effects");
  }
}
...
var bridgeDtsSchemas = [
  ...
  ["SkillExpansionFacts", schemas.SkillExpansionFactsSchema],
  ["SkillMessageDetails", schemas.SkillMessageDetailsSchema],
  ["SkillMessage", schemas.SkillMessageSchema],
  ["BridgeWarning", schemas.BridgeWarningSchema],
  ["SkillExpansionEffects", schemas.SkillExpansionEffectsSchema],
...
// src/version.ts
var taumelVersion = "0.0.347-g99856b08023b";
...
// src/skills.ts
function planSkillExpansion(core, facts) {
  const effects = decodeSkillExpansionEffects(core.call("planSkillExpansion", [facts]));
...
function notifyWarnings(plan, ctx) {
...
async function applySkillExpansionPlan(plan, destination) {
...
function installSkillResolver(pi, core) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (bypassCount > 0) {
      if (bypassCount === 1)
        bypassOnce.delete(text);
      else
        bypassOnce.set(text, bypassCount - 1);
    ...
    const outcome = await applySkillExpansionPlan(plan, {
      ...
      sendText: async (plannedText) => {
        ...
        try {
          ...
          if (count === 1)
            bypassOnce.delete(plannedText);
          else if (count > 1)
            bypassOnce.set(plannedText, count - 1);
          throw error;
        }
      }
    });
...
function stepThinkingLevel(pi, core, ctx, delta) {
  ...
  for (let nextIndex = index + delta;nextIndex >= 0 && nextIndex < thinkingLevels.length; nextIndex += delta) {
    ...
    if (currentThinkingLevel(pi) !== before)
      break;
  }
