...
val config_of_profile :
  ...
  ?no_sandbox:bool ->
  ?isolated_child:bool ->
  Capability_profile.t ->
  (config, string) result

val fail_closed_config :
  workspace_roots:string list -> isolated_child:bool -> config
...
val validate_resolved_workspace_mutation_paths :
  workspace_roots:string list ->
  resolved_mutation_path list ->
  (unit, string) result

val authorize_path :
  ?auth_path:string ->
  ?auth_roots:string list ->
  config ->
  path_access ->
  string ->
  decision
...
val temp_root_candidates : tmp_dir:string -> env_tmp_dir:string -> string list
...
val render_exec_result : ?diagnostic:failure_diagnostic -> exec_result -> string
...
val approval_prompt_outcome_of_string : string -> approval_prompt_outcome option

val approval_denial_message : approval_prompt_outcome -> string

val exec_approval_outcome : outcome:approval_prompt_outcome -> approval_outcome

val filesystem_approval_prompt : tool:string -> path:string -> approval_prompt
...
val apply_patch_to_map :
  ...
  string ->
  (string Patch.String_map.t, string) result
