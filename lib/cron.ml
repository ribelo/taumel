...
type delivery = { task : task; coalesced : int; content : string }

type startup_reason = New | Reload | Resume | Startup | Fork | Other

type startup_plan = { state : state; notify : bool; message : string }
...
let update_task_cron ~now id cron state =
  Result.bind (next_due cron ~now) (fun due ->
      ...
      | Some task ->
          Ok
            (replace_task
               { task with cron; next_due = due; pending_since = None }
               state))
...
let startup_message count =
  Printf.sprintf
    "%d stored cron task%s exist in this session. Cron is disabled on startup; \
     run /cron enable to arm them."
    count
...
let apply_startup reason state =
  ...
  | (Resume | Startup | Fork), _ :: _ ->
      {
        state = { state with enabled = false };
        notify = true;
        message = startup_message (List.length state.tasks);
      }
    ...
    let tasks =
      List.map
        (fun task ->
          ...
          | None when now >= task.next_due ->
              { task with pending_since = Some task.next_due }
          | None -> task)
        state.tasks
    in
    { state with tasks }
...
let deliverable facts task =
  ...
  | None -> false
  | Some _ -> (
      task.enabled && facts.host_idle && (not facts.plan_driving)
  ...
  if not state.enabled then None
    ...
    |> Option.map (fun task ->
        ...
        in
        {
          task;
          coalesced =
            count_occurrences task.cron ~from_time:pending_since ~until_time:now;
          content = task.prompt;
        })
...
let task_of_json json =
  ...
  if not (valid_task_id id) then
    ...
    match pending_since with Some value -> value <> next_due | None -> false
