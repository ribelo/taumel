...
type hunk =
  | Add_file of { path : string; contents : string }
...
type file_action = Write_file of string | Delete_path of string
...
let split_content_lines text =
  match List.rev (split_lines text) with
  | "" :: rest -> List.rev rest
...
let normalize_patch_path path =
  ...
  if
    String.starts_with ~prefix:"a/" path || String.starts_with ~prefix:"b/" path
...
let parse_chunks lines start stop =
  ...
  let rec outer acc index =
    if index >= stop then (List.rev acc, index)
      ...
      else if not (String.starts_with ~prefix:"@@" line) then
        outer acc (index + 1)
      else
  ...
  let apply_chunk lines chunk =
    ...
    Result.bind result (fun () ->
        ...
        | old_lines -> (
            ...
            let found =
              find_subsequence ~eof:chunk.is_end_of_file ~from:!cursor !pattern
                lines
            in
            let found =
            ...
            | Some index ->
                let next =
                  replace_subsequence lines ~index
                    ~old_length:(List.length !pattern) ~replacement:!replacement
                in
...
let parse_git_patch text =
  ...
  let rec parse_file acc index =
    let rec skip_blank index =
      if index < length && String.trim (line index) = "" then
        skip_blank (index + 1)
      else index
    in
    ...
    if index >= length then
      ...
      let rec read_headers index =
        if index >= length then index
          ...
          if
            index + 1 < length
            && String.starts_with ~prefix:"+++ " (line (index + 1))
          then (
      ...
      in
      let to_path =
        Option.value !rename_to
          ~default:(Option.value !new_path ~default:from_path)
      in
  ...
  let rec parse_hunks acc index =
    if index >= stop then
      ...
      let move_to, index =
        if
          index < stop
          && String.starts_with ~prefix:"*** Move to: " (line index)
        then
          ( Some
              (line index |> drop_prefix "*** Move to: " |> normalize_patch_path),
            index + 1 )
        else (None, index)
      in
...
let apply_to_map files hunks =
  let apply_hunk files = function
    ...
    | Update_file { path; move_to; chunks } -> (
        ...
        | Some original -> (
            let updated =
              if chunks = [] then Ok original
              else derive_updated_content original chunks
            in
            match updated with
  ...
  List.fold_left
    (fun result hunk ->
      match result with
      | Error _ as error -> error
      | Ok files -> apply_hunk files hunk)
    (Ok files) hunks
